...
class FeatureExtractor:
    def __init__(self):
        # මොඩල් එක කෙලින්ම ලෝඩ් කරන්න
        self.nlp = spacy.load("en_core_web_sm")

    # (ඉතිරි කෝඩ් එක ඒ විදිහටම තියන්න)
    ...
    def extract_ner_counts(self, text):
        ...
        for ent in doc.ents:
            if ent.label_ == "PERSON":
                ...
                gpe_count += 1
        return {"count_person": person_count, "count_org": org_count, "count_gpe": gpe_count}

    def get_combined_features(self, text):
        return {**self.extract_regex_features(text), **self.extract_ner_counts(text)}
